          ...
          <div ref={menuRef} className="relative">
            ...
            {open ? (
              <div role="menu" className="absolute right-0 z-50 mt-2 w-48 rounded-md border bg-popover p-1 text-popover-foreground shadow-md">
                <div className="cursor-default select-none rounded-sm px-2 py-1.5 text-sm opacity-60">{user?.email ?? 'Not signed in'}</div>
                <div className="-mx-1 my-1 h-px bg-border" />
                {user ? (
                  <button className="w-full rounded-sm px-2 py-1.5 text-left text-sm hover:bg-accent" onClick={() => { setOpen(false); void signOut() }}>Sign out</button>
                ) : (
                  <Link href="/login" className="block w-full rounded-sm px-2 py-1.5 text-left text-sm hover:bg-accent" onClick={() => setOpen(false)}>Sign in</Link>
                )}
              </div>
            ) : null}
          </div>
